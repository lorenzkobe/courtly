...
export async function runCompleteBookingsJob(): Promise<{
  ...
  const batchLimit = Math.min(
    Math.max(
      Number.parseInt(process.env.BOOKING_COMPLETION_BATCH_LIMIT ?? "", 10) ||
        DEFAULT_BATCH_LIMIT,
      1,
    ),
    MAX_BATCH_LIMIT,
  ...
  const seedLimit = Math.max(
    Number.parseInt(process.env.BOOKING_COMPLETION_SEED_LIMIT ?? "", 10) ||
      DEFAULT_SEED_LIMIT,
    batchLimit * 2,
  );
  ...
  const groupIds = [
    ...new Set(seedRows.map((row) => row.booking_group_id).filter(Boolean)),
  ] as string[];
  ...
  for (const rows of groupMap.values()) {
    ...
    if (!hasReachedEnd(finalRow.date, finalRow.end_time, nowDate, nowTime)) {
      continue;
    }
    entities.push({
  ...
  const { selected, hasMore: hasMoreEntities } = pickEntityBatch(
    entities,
    batchLimit,
  );
